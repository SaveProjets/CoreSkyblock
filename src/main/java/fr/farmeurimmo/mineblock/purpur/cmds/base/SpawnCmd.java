...
public class SpawnCmd implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command cmd, @NotNull String label, @NotNull String[] args) {
        if (args.length == 1) {
            if (sender.hasPermission("MineBlock.admin")) {
                ...
                target.teleportAsync(MineBlock.SPAWN).thenRun(() -> {
        ...
        p.teleportAsync(MineBlock.SPAWN).thenRun(() -> p.sendActionBar(Component.text("§aVous avez été téléporté au spawn.")))
                .exceptionally(throwable -> {
